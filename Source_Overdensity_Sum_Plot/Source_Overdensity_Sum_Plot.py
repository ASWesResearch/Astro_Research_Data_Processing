...
def Source_Histogram_Sum_Plot(Gname_L,Fileout_B=True,Outpath=False):
    """
    ...
    if not os.path.exists(directory):
        os.makedirs(directory)
    """
    plt.savefig(Path_Sum+'Sum_D25_Histogram.png') #Saves angluar histogram figure
    ...
    if(Fileout_B):
        #O_R_fname=path_Obs_ID + Gname_Modifed+'_'+str(Obs_ID)+'_Overdensity_Ratios.txt'
        file=open(Path_Sum+'Sum_D25_Data.png',"w")
        #print "O_R_fname:",O_R_fname
        #D25_Line="D25:"+str(D25_S_Maj_Arcmin)+"|\n"
        #file.write(D25_Line)
        #BG_Source_A=np.array(BG_Source_L)
